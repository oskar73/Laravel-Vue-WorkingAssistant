...
class FrontController extends AdminController
{
    ...
    public function index()
    {
        ...
        return view(self::$viewDir.'directory.front', compact('front', 'name'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $path = config('custom.storage_name.directory');

        if ($nav_image = $request->nav_image) {
            $navimage_name = 'directory_nav.'.$nav_image->getClientOriginalExtension();

            $data['nav_image'] = BaseModel::fileUpload($nav_image, $navimage_name, $path);
        }
        if ($meta_image = $request->meta_image) {
            $metaimage_name = 'directory_meta.'.$meta_image->getClientOriginalExtension();
