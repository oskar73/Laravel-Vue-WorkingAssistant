...
class FrontController extends AdminController
{
    ...
    public function index()
    {
        ...
        return view(self::$viewDir.'siteAds.front', compact('front', 'name'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $path = config('custom.storage_name.siteAds');

        if ($nav_image = $request->nav_image) {
            $navimage_name = 'siteAds_nav.'.$nav_image->getClientOriginalExtension();

            $data['nav_image'] = BaseModel::fileUpload($nav_image, $navimage_name, $path);
        }
        if ($meta_image = $request->meta_image) {
            $metaimage_name = 'siteAds_meta.'.$meta_image->getClientOriginalExtension();
