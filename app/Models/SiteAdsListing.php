...
class SiteAdsListing extends BaseModel implements HasMedia
{
    use InteractsWithMedia;

    protected $table = 'site_ads_listings';
    ...
    public function storeRule($request, $spot, $price)
    {
        ...
        $rule['url'] = 'nullable|max:255|regex:/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/';
        if ($request->cta_check) {
            $rule['cta_url'] = 'required|max:255|regex:/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/';
            $rule['cta_type'] = 'required|string|in:buy_now,shop_now,visit_now,learn_more,join_now';
        }
        ...
        $rule['customer'] = 'required|exists:users,id,status,active,web_id,'.tenant('id');
    ...
    {
        return $this->hasMany(SiteAdsListingTrack::class, 'listing_id');
    }
    ...
    {
        return $this->hasMany(SiteAdsEvent::class, 'listing_id')->orderBy('start_date');
    }

    public function track($request)
    {
        $track = new SiteAdsListingTrack();
    ...
    public function getDatatable($status, $user)
    {
        switch ($status) {
            case 'pending':
                $listings = $this::with('user', 'spot.page', 'events')->where('status', 'pending');
                break;
            case 'all':
                $listings = $this::with('user', 'spot.page', 'events');
                break;
            case 'approved':
                $listings = $this::with('user', 'spot.page', 'events')->where('status', 'approved');
                break;
            case 'denied':
                $listings = $this::with('user', 'spot.page', 'events')->where('status', 'denied');
                break;
            case 'expired':
                $listings = $this::with('user', 'spot.page', 'events')->where('status', 'expired');
                break;
            case 'new':
                $listings = $this::with('user', 'spot.page', 'events')->where('status', 'new');
                break;
        }
        if ($user != 'all') {
        ...
        return Datatables::of($listings)->addColumn('checkbox', function ($row) {
            ...
            if ($price->type === 'impression') {
                $result = 'Total Impressions: '.$row->impression_number."<br> <a href='".route('admin.siteAds.listing.tracking', $row->id)."' class='underline'>Current Impressions: {$row->current_number}</a>";
            } else {
                $result = 'Duration:<br>';
                foreach ($row->events as $event) {
                    $result .= $event->start_date.' ~ '.$event->end_date.'<br>';
                }
                $result .= "<a href='".route('admin.siteAds.listing.tracking', $row->id)."' class='underline'>Current Impressions: {$row->current_number}</a>";
            }
            ...
            return '
                    <a href="'.route('admin.siteAds.listing.show', $row->id).'" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill tooltip_3" title="Detail">
                            <i class="la la-eye"></i>
                    </a>
                    <a href="'.route('admin.siteAds.listing.edit', $row->id).'" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill tooltip_3" title="Edit">
    ...
    {
        $listings = $this::with('user', 'spot.page', 'events')->where('user_id', user()->id);
        ...
        return Datatables::of($listings)->addColumn('spot', function ($row) {
            ...
            $price = json_decode($row->price);
            if ($price->type === 'impression') {
                $result = 'Total Impressions: '.$row->impression_number."<br> <a href='".route('user.siteAds.tracking', $row->id)."'  class='underline'>Current Impressions: {$row->current_number}</a> ";
            } else {
                $result = 'Duration: <br>';
                foreach ($row->events as $event) {
                    $result .= $event->start_date.' ~ '.$event->end_date.'<br>';
                }
                $result .= "<a href='".route('user.siteAds.tracking', $row->id)."' class='underline'>Current Impressions: {$row->current_number}</a>";
            }
            ...
            if ($row->status == 'expired') {
                return '
                    <a href="'.route('user.siteAds.detail', $row->id).'" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill tooltip_3" title="Detail">
                            <i class="la la-eye"></i>
                    </a>';
            } else {
                return '
                    ...
                    <a href="'.route('user.siteAds.edit', $row->id).'" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill tooltip_3" title="Edit">
                            <i class="la la-edit"></i>
                    </a>';
            }
