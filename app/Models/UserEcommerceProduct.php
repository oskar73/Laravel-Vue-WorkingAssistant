...
class UserEcommerceProduct extends BaseModel
{
    ...
    {
        return $this->belongsTo(Module\EcommerceOrderItem::class, 'order_item_id')->withDefault();
    }
    ...
    {
        $userProduct = new UserEcommerceProduct();
        ...
        $userProduct->price = $item['parameter']['price'];
        $userProduct->size = $item['parameter']['size'];
        $userProduct->color = $item['parameter']['color'];
        $userProduct->custom = $item['parameter']['custom'];
        $userProduct->save();
