...
if (!function_exists('asset')) {
    ...
    {
//        return app('url')->asset($path, $secure) . '?v=' . config('app.version') . time();
    ...
    function moduleName($type)
    {
        switch ($type) {
            ...
            case 'blogAds':
                ...
                break;
            case 'siteAds':
                $result = 'Site Advertisement Spot';
                break;
            case 'directoryAds':
                $result = 'Directory Advertisement Spot';
                break;
            case 'directoryPackage':
                $result = 'Directory Listing Package';
                break;
            case 'ecommerce':
...
if (!function_exists('makedir')) {
    ...
    {
        if (!file_exists($path)) {
            File::makeDirectory($path, $mode = $perm, true, true);
        }
    ...
    function tenant($key = null)
    {
        if (app()->has('tenant')) {
            try {
                $tenant = app('tenant');
                if ($key == 'c_modules') {
                    return $tenant->getModules->where('status', 1)->pluck('slug')->toArray();
                }
                if ($key == 'p_modules') {
                    return $tenant->getModules->where('status', 1)->where('publish', 1)->pluck('slug')->toArray();
                }

                return $key == null ? $tenant : $tenant->$key ?? '';
            } catch (\Exception $e) {
                return null;
            }
        }
...
if (!function_exists('primary_option')) {
    function primary_option($key = null, $default = null)
    {
        if (is_null($key)) {
            return app('primary_option');
        }

        if (is_array($key)) {
            return app('primary_option')->set($key);
        }

        return app('primary_option')->get($key, $default);
    }
}

if (!function_exists('primary_option_exists')) {
    function primary_option_exists($key)
    {
        return app('primary_option')->exists($key);
    }
}
...
if (!function_exists('get_before')) {
    ...
    {
        $date = now()->sub($period, $period_unit)->addDay();
    ...
    function todoTypeToName($type)
    {
        ...
        switch ($type) {
            ...
            case 'blogAdsListing':
                ...
                break;
            case 'siteAdsListing':
                $result = 'Site Ads Listings';
                break;
            case 'directoryAdsListing':
                $result = 'Directory Ads Listings';
                break;
...
if (!function_exists('getTimezoneList')) {
    function getTimezoneList($data = null)
    {
        $str = '<select ' . $data . '>
...
if (!function_exists('builder_assets')) {
    function builder_assets($path = ''): string
    {
        $builder_assets_domain = getenv('BUILDER_ASSETS_DOMAIN');
        return $builder_assets_domain . $path;
    }
}

if (!function_exists('websiteOwner')) {
