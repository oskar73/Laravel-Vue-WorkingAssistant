...
@section('breadcrumb')
    ...
    <div class="col-md-6 text-right">
        <a href="{{route('user.purchase.order.index')}}" class="btn m-btn--square  btn-outline-info m-btn m-btn--custom">
            Back
        </a>
    </div>
@endsection
                        ...
                        @foreach($order->items as $item)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <a href="{{route('user.purchase.subscription.detail', $item->id)}}">${{$item->getRecurrentPrice()}}</a>
                                    @endif
                                </td>
...
@endsection
@section('script')
    <script src="{{asset('assets/js/user/purchase/orderDetail.js')}}"></script>
@endsection
