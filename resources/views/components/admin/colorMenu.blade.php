<div class="table-responsive">
    <table class="table table-hover ajaxTable datatable {{$selector}}">
        ...
        <tbody>
        @foreach($menuColors as $color)
            <tr>
                <td><input type="checkbox" class="checkbox" data-id="{{$color->id}}"></td>
                <td>{{$color->name}}</td>
                <td><div class="color_disp_area" style="background-color:#{{$color->themeColor("nav_bg")}}"></div></td>
                <td><div class="color_disp_area" style="background-color:#{{$color->themeColor("nav_item")}}"></div></td>
                <td><div class="color_disp_area" style="background-color:#{{$color->themeColor("nav_hover_bg")}}"></div></td>
                <td><div class="color_disp_area" style="background-color:#{{$color->themeColor("nav_hover_item")}}"></div></td>
                <td>
                    @if($color->default)
                        <span class="c-badge c-badge-success">Current</span>
                    @else
                        <a href="javascript:void(0);" class="c-badge c-badge-success hover-vis hover-box switchOne" data-action="current" data-type="menu">Set as Current?</a>
                    @endif
                </td>
