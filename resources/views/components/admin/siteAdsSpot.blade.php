    ...
    <table class="table table-hover ajaxTable datatable {{$selector}}">
        ...
        @foreach($spots as $spot)
            <tr>
                ...
                <td>{{$spot->name}}</td>
                @if($display==1)
                    <td>{{$spot->page->name}}</td>
                    <td>
                        {{$spot->parentType->name}} ({{$spot->parentType->width}} x {{$spot->parentType->height}} )
                    </td>
                @endif
                <td>
                    @if($spot->position_id)
                        <span class="c-badge c-badge-success">Selected</span>
                    @endif
                        <a href="{{route('admin.siteAds.spot.editPosition', $spot->id)}}" class="hover-none">
                ...
                <td>
                    <a href="{{route('admin.siteAds.spot.edit', $spot->id)}}"
