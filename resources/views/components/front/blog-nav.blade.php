<div>
    ...
    <div class="page-nav-info bg-theme" x-data="{c_modal:false,c_cat:{img:null,name:null,slug:null,desc:null},t_modal:false,s_modal:false}">
        <ul class="list-style-none text-center">
            ...
            @if(in_array("blogAds", tenant("p_modules")))
                <li class="d-inline-block m-2"><a href="{{route('blogAds.index')}}" class="text-white">Blog Ads</a></li>
            @endif
            @if(in_array("advanced_blog", tenant("p_modules"))&&$packageShow)
                <li class="d-inline-block m-2"><a href="{{route('blog.package')}}" class="text-white">Blog Package</a></li>
            @endif
        </ul>
        <template
            x-if="s_modal"
        ...
        >
            ...
            >
                ...
                <div class="container">
                    ...
                    @foreach($tags as $tag)
                        <a href="{{route('blog.tag', $tag->slug)}}" class="btn rounded-0 btn-outline-info m-1">{{$tag->name}}</a>
                    @endforeach
                </div>
            </div>
        </template>
