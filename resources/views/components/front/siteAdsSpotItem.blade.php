        ...
        <table class="table table-hover ajaxTable datatable">
            ...
            @foreach($items as $item)
                <tr>
                    ...
                    <td class="text-left">{{$item->page->name}}</td>
                    <td class="text-left">
                        (Width:{{$item->parentType->width}}px, Height:{{$item->parentType->height}}px)
                    </td>
                    <td class="text-right">
                    ...
                    <td class="text-center">
                        <a href="{{route('siteAds.spot', $item->slug)}}" class="btn border-radius-none btn-outline-success outline_custom_btn">Select</a>
                    </td>
                </tr>
            @endforeach
            </tbody>
            ...
            @forelse($items as $item)
                <div class="col-lg-4 mb-5">
                    <div class="project-grid-style3">
                        <a href="{{route('siteAds.spot', $item->slug)}}" class="inner-box">
                            ...
                            <div class="project-desc  pb-0">
                                ...
                                <span class="mb-2">({{$item->page->name}})</span> <br>
                                <div class="mt-3">
                                    (Width:{{$item->parentType->width}}px, Height:{{$item->parentType->height}}px)
                                </div>
